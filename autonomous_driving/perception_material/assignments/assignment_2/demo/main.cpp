...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        if (longest.id != -1) {
            renderer.addText(0.0, -4.0, longest.id + 1000);  // Mostra solo l'ID + 1000
            
            // Opzionalmente, possiamo mostrare la distanza in una posizione diversa
            renderer.addText(0.2, -2.0, static_cast<int>(longest.distance * 100));  // Moltiplichiamo per 100 per mostrare i centimetri come intero
        }
