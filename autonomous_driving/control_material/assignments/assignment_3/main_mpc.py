...
exercise_id = 2
sim_speed   = 25.0   # (10.0 o 15.0, 20.0, 25.0, ...)

# Creazione della cartella di output per i plot e i dati
out_folder = f"results/Exercise_{exercise_id}_{int(sim_speed)}_ms_mpc"
...
sim_time   = 72.55    # Impostare tempo di simulazione in secondi
                    # Per esercizio 1.1: 170 secondi
                    # Per esercizio 1.2: 115 secondi
                    # Per esercizio 2.1: 87.52 secondi
...
def run_simulation(ax_fallback, steer_fallback, dt, integrator, model, steps=500):
    ...
    for step_i in range(steps):
        ...
        steer_cmd = opt_step(targets, sim)
        steer_cmd = float(steer_cmd)

        # 4) integrazione
        sim.integrate(ax_pid, steer_cmd)
