...
exercise_id = 2
sim_speed   = 25.0   # (o 15.0, 20.0, 25.0, ...)

# Creazione della cartella di output per i plot e i dati
out_folder = f"results/Exercise_{exercise_id}_{int(sim_speed)}_ms_stanley"
...
sim_time   = 72.71    # Impostare tempo di simulazione in secondi
                    # Per esercizio 1.1: 170 secondi
                    # Per esercizio 1.2: 115 secondi
                    # Per esercizio 2.1: 87.625 secondi
...
def run_simulation(ax_fallback, steer_fallback, dt, integrator, model, steps=500):
    ...
    for step_i in range(steps):
        ...
        steer_cmd = stanley_controller.compute_steering_angle(actual_pose, stanley_target, sim.vx)
...
def main():
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(os.path.join(out_folder, "SideSlipAngle.png"), dpi=150)
    plt.close()
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(os.path.join(out_folder, "LongitudinalAcceleration.png"), dpi=150)
    plt.close()
