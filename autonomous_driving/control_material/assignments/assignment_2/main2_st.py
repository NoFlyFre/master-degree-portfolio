...
k_stanley = 1.0  # Gain for cross-track error for Stanley
...
def run_simulation(ax, steer, dt, integrator, model, steps=500):
    ...
    for step in range(steps):
        ...
        # Stanley
        front_axle_pos = (sim.x + lf*math.cos(sim.theta),
                        sim.y + lf*math.sin(sim.theta),
                        sim.theta)
        stanley_target = (position_projected[0],
                        position_projected[1],
                        path_spline.calc_yaw(path_spline.cur_s))

        steer_cmd = stanley_controller.compute_steering_angle(front_axle_pos, stanley_target, sim.vx)
